...
### מה מצפים לקבל:
- **קידוד מקיף ומפורט**: זהה את כל הקודים הרלוונטיים לשאלת המחקר
- **ציטוטים רבים ומדויקים**: לכל קוד לפחות 2-3 ציטוטים עם חותמות זמן
- **כיסוי מלא**: ודא שכל התוכן הרלוונטי מקודד, לא רק חלקים בולטים
...
### מה מצפים לקבל:
- **קיבוץ מקיף**: כל הקודים הרלוונטיים מקובצים לתימות קוהרנטיות
- **כל הציטוטים נשמרים**: ודא שכל הציטוטים מהקודים מועברים לתימות
- **תימות מפורטות**: כל תימה כוללת תיאור ברור והסבר על הקוהרנטיות שלה
- **כיסוי מלא**: כל הקודים הרלוונטיים לשאלת המחקר נכללים בתימות
"""
...
### מה מצפים לקבל - דו"ח מורחב ומפורט:
- **תקציר מנהלים מורחב**: סיכום מקיף של הממצאים העיקריים (5-7 משפטים לפחות)
- **הגדרות תימות מפורטות**: כל תימה עם הגדרה אקדמית ברורה ומפורטת (2-3 משפטים)
- **קשרים בין תימות מפורטים**: זיהוי ופירוט מעמיק של הקשרים, החפיפות, והסתירות בין התימות
- **השלכות תיאורטיות מורחבות**: ניתוח מעמיק של המשמעות התיאורטית והתרומה להבנת התופעה
- **השלכות מעשיות מפורטות**: המלצות מעשיות ברורות וישימות ליישום בשטח
- **מגבלות מפורטות**: רשימה מקיפה של כל המגבלות (גודל מדגם, הקשר, שיטה, מגבלות הניתוח)
- **כיווני מחקר עתידיים מורחבים**: המלצות ספציפיות ומפורטות למחקרים עתידיים
- **כל הציטוטים נשמרים**: ודא שכל הציטוטים עם חותמות הזמן נשמרים בכל התימות

**חשוב: וודא שכל הציטוטים נשמרים עם חותמות זמן והדו"ח מקיף ומורחב!**
"""
